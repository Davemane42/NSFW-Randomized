...
class RandomizerData:
    default_negative = "lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry, artist name, ((extra fingers)), ((poorly drawn hands)), ((poorly drawn face)), (((mutation))), (((deformed))), ((bad anatomy)), (((bad proportions))), ((extra limbs)), glitchy, ((extra hands)), ((mangled fingers)), six fingers, 6 fingers, four fingers, 4 fingers, 3 fingers, three fingers"

    beautifier = ("aesthetic", "attractive", "beauteous", "bonny", "bonnie", "comely", "cute", "drop-dead", "fair",
        ...
        "fascinating", "glamorous", "glamorous", "prepossessing", "elegant", "exquisite", "glorious",
        "Junoesque", "magnificent", "resplendent", "splendid", "sublime", "superb", "flirty",
        "flawless", "perfect", "radiant", "dainty", "delicate", "personable", "pleasant",
        "presentable", "pretty", "desirable", "dishy", "dollish", "foxy", "hot",
    ...
    # Take care to ensure that what you add here makes grammatical sense
    accessory = ("wearing nothing", "wearing stockings", "wearing a choker", "with a pony tail", "with freckles",
                "with a devil tail", "with a butt plug", "with a vibrator", "with an anal hook", "in shibari bondage",
                "with a whip", "with a strap-on", "with a huge dildo", "with cybernetic augments", "wearing halo armor",
                "wearing a choker and eyeshadow", "with a bodysuit", "with a mecha suit", "with tribal tattoos",
                "with bodypaint", "with a beautiful ass")

    # "breasts" is automatically appended to these tokens
    bSize = ("large", "medium", "small")

    # "expression" is automatically appended to these tokens
    expression = ("surprised", "angry", "happy", "satisfied", "bored", "excited", "calm", "smiling", "orgasmic",
                "blissful", "zen", "horny", "sinister", "seductive")

    # "hair" is automatically appended to these tokens
    hairColor = ("white", "black", "purple", "pink", "red", "blue", "neon",
                "glowing", "vibrant colorful", "bright green", "fire")

    # Only include length here
    hairLength = ("long", "medium", "short")

    # "eyes" is automatically appended to these tokens, use only colors
    eyeColor = ("hazel", "green", "brown", "black", "grey", "purple", "pink", "red", "silver", "yellow", "Sapphire",
                "turquoise", "amethyst", "aquamarine", "ruby", "coral", "peach", "gold", "dark brown", "jade",
                "teal", "ice blue", "light blue", "blue", "grey-blue", "amber", "crimson", "fiery", "glowing")

    # Take care to ensure that what you add here makes grammatical sense
    place = ("in ancient greece", "in far out space", "on an alien planet", "in japan", "in a sauna", "in a steam room",
            "at the spa", "at a lake", "at a river", "at yosemite", "in china", "in korea", "on a spaceship", "in hell",
            "at the beach", "in a forest", "at the golden gate bridge", "at an onsen", "on a snowy mountain",
            "at a temple", "in a locker room", "in the bedroom", "in a dungeon", "at a castle", "sc-fi setting",
            "in a cyberpunk alley", "in europe", "in a public park", "fantasy rpg", "in the desert", "in the bath",
            "in the shower", "in a swiss chalet", "by a fireplace", "at atlantis", "at burning man", "in the kitchen",
            "at a night club", "on a yacht", "on a cruise ship", "at the pool", "in a rainforest", "by a waterfall",
            "in an alpine forest", "on a rainy street", "at lake tahoe", "at a rave", "in a hotspring")

    # What is this woman doing???
    action = ("having missionary sex", "having doggy-style sex", "giving a blowjob", "having vaginal sex",
            "having anal sex", "stretching", "dancing", "masturbating", "having group sex", "pissing", "peeing",
            "lactating")

    # Combines `action` with the this. "with" is automatically prepended to these tokens
    perp = ("a tentacle monster", "a group of men", "a group of aliens", "a sex machine", "a giant dildo",
            "a muscly man", "a demon", "a slime monster", "a water elemental", "an anal vibrator", "a dragon")

    # Type of woman
    race = ("catgirl", "steamy", "mature", "hot", "foxgirl", "slime girl", "nymph", "angel", "batgirl",
            "slut", "beautiful", "princess", "gothic", "emo", "succubus", "medusa", "pixie", "mermaid",
            "teacher", "gundam", "schoolgirl", "milf", "prostitute", "elf eared", "galactic", "knight",
            "ninja", "mage", "cyborg", "snake", "dragon", "futanari", "wolfgirl", "demon", "devil",
            "butterfly", "slime", "pirate")
    ...
    artStyle = ("60s kitsch and psychedelia", "abstract illusionism", "ambrotype",
                ...
                "stuckism", "symbolism", "the matrix", "tilt-shift", "tintype", "tonalism")

    # Framing of the image
    distance = ("extreme closeup", "closeup", "portrait", "half body shot", "full-body", "full body",
                "wide angle", "distance shot")

    # Optional token appended individually towards the end of the prompt.
    ...
    # Optional token appended at the end of the prompt. "by" is prepended to these.
...
class Script(scripts.Script):
    ...
    def title(self):
        return "NSFW Randomizer V2"

    def ui(self, is_img2img):
        with gr.Box():
            with gr.Row():
                num_prompts = gr.Slider(label='Number of prompts', minimum=1, maximum=64, step=1, value=1)
                max_batch = gr.Slider(label='Max images per batch', minimum=1, maximum=32, step=1, value=1)
                prompt_seed = gr.Number(label='Force seed for prompt generation', precision=0, value=-1)

            with gr.Row():
                use_artists = gr.Checkbox(label="Use artists modifier in prompts", value=False)
                use_artstyle = gr.Checkbox(label="Use ArtStyle modifier in prompts", value=False)
                use_details = gr.Checkbox(label="Use Details modifier in prompts", value=False)
                use_beautifier = gr.Checkbox(label="Use Beautifier modifier in prompts", value=False)
            
            with gr.Row():
                prompt_start = gr.Textbox(label='Prompt Start', value="")
            
            with gr.Row():
                static_random = gr.Checkbox(label="Generate same prompt given a seed", value=True)
                lower_bound = gr.Slider(label='Tokens strenght min', minimum=1, maximum=2, step=0.05, value=1)
                upper_bound = gr.Slider(label='Tokens strenght max', minimum=1, maximum=2, step=0.05, value=1.5)
                 

        return [num_prompts, max_batch, prompt_seed, use_artists, use_artstyle, use_details, use_beautifier, prompt_start, static_random, lower_bound, upper_bound]
    
    def show(self, is_img2img):
        return True

    def run(self, p, num_prompts, max_batch, prompt_seed, use_artists, use_artstyle, use_details, use_beautifier, prompt_start, static_random, lower_bound, upper_bound):
        debug = False
        data = RandomizerData()
        a = lower_bound
        b = upper_bound

        p.n_iter = math.ceil( (num_prompts) / max_batch )
        p.batch_size = max_batch

        new_len = p.batch_size*p.n_iter
        
        all_prompts = [None] * new_len

        if p.negative_prompt == "":
            p.negative_prompt = data.default_negative

        p.seed = int(get_fixed_seed(p.seed))
        all_seeds = [x for x in range(p.seed, p.seed+new_len)]

        if debug: print(f'{f" NSFW Randomizer V2 ":=^35}')
        use_prompt_seed = (prompt_seed == -1 or prompt_seed == None) == False

        def token_pow(a, b):
            return f"{random.uniform(a, b):.2f}"

        for cur_prompt in range(new_len):

            # pick same random seed for a given seed
            if (static_random):
                if use_prompt_seed:
                    random.seed(prompt_seed)
                else:
                    random.seed(all_seeds[cur_prompt])
            else:
                random.seed(time.time())

            token_list = []

            if use_beautifier: token_list.append(f"({random.choice(data.beautifier)}:{token_pow(a, b)}), ")

            token_list.append(f"({random.choice(data.distance)}:{token_pow(a, b)}), ")
            token_list.append(f"({random.choice(data.race)}:{token_pow(a, b)}), ")
            token_list.append(f"(woman with {random.choice(data.bSize)} breasts:{token_pow(a, b)}) ")
            token_list.append(f"and ({random.choice(data.hairLength)} ")
            token_list.append(f"{random.choice(data.hairColor)} hair:{token_pow(a, b)}) ")
            token_list.append(f"and ({random.choice(data.eyeColor)} eyes:{token_pow(a, b)}) ")
            token_list.append(f"({random.choice(data.accessory)}:{token_pow(a, b)}) ")
            token_list.append(f"({random.choice(data.action)}:{token_pow(a, b)}) ")
            token_list.append(f"with ({random.choice(data.perp)}:{token_pow(a, b)}), ")
            token_list.append(f"({random.choice(data.place)}:{token_pow(a, b)}), ")
            token_list.append(f"({random.choice(data.expression)} expression:{token_pow(a, b)}), ")
            token_list.append(f"(nsfw:{token_pow(a, b)}), ")
            token_list.append(f"(perfect face:{token_pow(a, b)}), ")
            token_list.append(f"(perfect fingers:{token_pow(a, b)})")

            if use_details: token_list.append(f", ({random.choice(data.details)}:{token_pow(a, b)})")
            if use_artstyle: token_list.append(f", ({random.choice(data.artStyle)}:{token_pow(a, b)})")
            if use_artists: token_list.append(f", by ({random.choice(data.artists)}:{token_pow(a, b)})")

            prompt = prompt_start + "".join(token_list)
            #for token in token_list:
            #    prompt += f"{'(' if '(' not in token else ''}{token}:{random.uniform(lower_bound, upper_bound):.2f}), "

            if debug: print(f'\n{f" Prompt #{cur_prompt} ":-^30}\n{prompt}')
            
            all_prompts[cur_prompt] = prompt

            if debug: print(f"\nSeed: {all_seeds[cur_prompt]}")
            
            if use_prompt_seed:
                all_prompts = [prompt] * new_len
                break

        p.prompt = all_prompts
        p.negative_prompt = [p.negative_prompt] * new_len
        p.seed = all_seeds
        if use_prompt_seed: p.extra_generation_params["Prompt Seed"] = prompt_seed
